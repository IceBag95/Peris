...
// Συνάρτηση που δείχνει το μενού επιλογών
void show_menu(){
...
// Συνάρτηση που κάνει έλεγχο εγκυρότητας στην επιλογή του χρήστη ότι όντως είναι μεταξέυ 1,2,3,4,5.
int users_choice(){
    int x;
    do {
        x = _int_input();
...
// ΣΗΜΕΙΩΣΗ: Επειδή ο πίνακας μπορεί να μην γεμίσει πλήρως, πρέπει να στείλω ΣΕ ΟΛΕΣ ΤΙΣ print που έφτιαξα, το τελικό μέγεθος της
//           λίστας. Αν δεν το κάνω οι θέσεις μνήμης που δεν έχουν γεμίσει, μπορεί να μου δημιουργήσουν πρόβλημα καθώς μπορεί να είναι
//           γεμισμένες με άλλες τιμές που δεν έχει βάλει ο χρήστης. Θα μπορούσα να τις γεμίσω στην αρχή όλες με 0 αλλά αυτό απλά 
//           καλύπτει το πρόβλημα. Οπότε αποφάσισα να στέλνω το μέγεθος της λίστας που όντως γέμισε από τον χρήστη ώστε να διαχειρίζομαι
//           κάθε φορά μόνο αυτό το κομμάτι.
