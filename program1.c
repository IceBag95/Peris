...
int main() {
    ...
    int counter = 1;
    int n = 2;  // αυτό μου δείχνει ποιον αριθμό ελέγχω αν είναι πρώτος. Ξεκινάω από το 2 γιατί είναι ο πρώτος "πρώτος".
    while (counter <= N) {
        int is_prime = 1;

        for (int i = 2; i <= sqrt(n); i++){ // το i μου διαλέγει αριθμούς πριν το i βλέπε εξήγηση παρακάτω **
        ...
        n++; // παίρνω για έλεχγο τον επόμενο αριθμό και ξανά η επανάληψη απο την αρχή.
    }
...
** Επεξήγηση:
    ...
    - Το n παίρνει διαδοχικά σε κάθε επανάληψη αριθμούς, ξεκινώντας από τον αριθμό 2
    ...
    - Παίρνω το i το οποίο ξεκινάει απο το 2 πάντα. Αυτό μου παίρνει όλους τους αριθμούς πίσω από την τετραγωνική ρίζα του 11 (εδώ είναι περίπου το 3.5)
    - Στην πρώτη επανάληψη για το i λέω είναι το 2 (από το οποίο ξεκινάω) μικρότερο του 3.5 (τετραγωνική ρίζα του 11)? Ναι. Διαιρεί το 2 ακριβώς το 11? Όχι. (καλο αυτο)
    - Άρα αυξάνω το i κατά 1 και πλέον είναι 3.
    ...
    - Το N δείχνει πόσους πρώτους αριθμούς ζήτησε να του βρούμε ο χρήστης (τό εβαλε ο ίδιος στην αρχή του προγράμματος). 

    Αυτή τη λογική την επαναλαμβάνουμε μέχρι το counter να φτάσει να είναι όσο το N. 
